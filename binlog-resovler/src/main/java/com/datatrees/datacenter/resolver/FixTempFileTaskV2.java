...
public class FixTempFileTaskV2 {
  ...
  public static void main(String[] args) {
    ...
    tempAvroFiles.forEach(file -> {
      ...
      if (args.length == 1) {
        if (args[0].equalsIgnoreCase(splitTargetPath[6])) {
          System.out.println(targetPath);
          fileStorage.commit(tempPath, targetPath);
        }
      } else if (args.length == 2) {
        if (args[0].equalsIgnoreCase(splitTargetPath[6]) &&
          args[1].equalsIgnoreCase(splitTargetPath[7])
          ) {
          System.out.println(targetPath);
          fileStorage.commit(tempPath, targetPath);
        }
      } else if (args.length == 3) {
        if (args[0].equalsIgnoreCase(splitTargetPath[6]) && args[1]
          .equalsIgnoreCase(splitTargetPath[7]) && String
          .format("%s/%s/%s", splitTargetPath[8], splitTargetPath[9], splitTargetPath[10])
