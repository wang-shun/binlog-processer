...
public class TransThread implements Serializable, Runnable {
  ...
  TransThread(String src, String dest, long startPos, long endPos, String fileName,
    String instanceId) {
    this.src = src;
    this.dest = dest;
  ...
  public void run() {
    LOG.info(
      "the current thread is: " + Thread.currentThread().getName() + " begin to download file :"
        + fileName);
    ...
    if (startPos == endPos && over) {
      ...
      try {
        List<Map<String, Object>> processRecord = DBUtil
          .query(TableInfo.BINLOG_PROC_TABLE, whereMap);
        if (processRecord.size() == 0) {
          ...
          try {
            ...
            TaskDispensor.defaultDispensor().dispense(new Binlog(path, instanceId + "_" + fileName,
