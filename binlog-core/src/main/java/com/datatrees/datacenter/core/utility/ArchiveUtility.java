...
public class ArchiveUtility {
    ...
    public static InputStream unArchive(String file, InputStream stream) {
        ...
        try {
            ...
            logger.error(e.getMessage(), e);
            throw new BinlogException(String.
                    format("error to get archiveEntry of %s because of %s", file, e.getMessage()), e);
        }
        try {
            ...
            while ((count = tarArchiveInputStream.read(buffer, 0, 1024)) != -1) {
                try {
                    ...
                    logger.error(e.getMessage(), e);
                    throw new BinlogException(String.
                            format("error to write stream of %s because of %s", file, e.getMessage()), e);
                }
            }
            ...
            logger.error(e.getMessage(), e);
            throw new BinlogException(String.
                    format("error to read stream of %s because of %s", file, e.getMessage()), e);
        } finally {
